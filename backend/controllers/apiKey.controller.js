...
// FIX (async gap flagged since Round 2): apiKey.service.js's createApiKey is
// async (queued, atomic-write pattern). Without await here, `created` was a
// Promise — `created.id` was undefined and `{ ...created }` spread to `{}`,
// so the response never actually contained the new key. The caller had no
// way to retrieve the one-time plaintext key at all.
exports.createApiKey = async (req, res, next) => {
  try {
    ...
    const created = await apiKeyService.createApiKey(req.user.id, name, { role, expiresInDays });
...
// FIX (async gap flagged since Round 2): revokeApiKey is also async now.
// Without await, `ok` was always a (truthy) Promise object, so `if (!ok)`
// never fired — the endpoint reported { success: true } even when the key
// didn't exist or belonged to someone else, and the revoke silently never
// happened.
exports.revokeApiKey = async (req, res, next) => {
  try {
    const ok = await apiKeyService.revokeApiKey(req.user.id, req.params.id);
